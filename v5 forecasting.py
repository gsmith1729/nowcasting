...
short_var = ['ACOGNO','ANDENOx.x','TWEXAFEGSMTHx.x','UMCSENTx.x','OUTMS','TCU','LNS13023621','LNS13023557','LNS13023705','LNS13023569','AWHNONAG','ACOGNOx','ANDENOx.y','INVCQRMTSPL','WPU0531','AHETPIx','COMPRMS','OPHMFG','ULCMFG','MORTGAGE30US','MORTG10YRx','REVOLSLx','DRIWCIL',  'USSTHPI','SPCS10RSA', 'SPCS20RSA', 'TWEXAFEGSMTHx.y', 'EXUSEU', 'USEPUINDXM', 'GFDEGDQ188S',  'GFDEBTNx',  'NASDAQCOM',  'CUSR0000SEHC']
...
endog_vars=monthly_var+[ "PCECC96","FPIx","PRFIx","OUTMS","FGRECPTx","GDPC1"]
endog=df.loc[start:end, endog_vars]
endog_train=endog.loc[start:endsample, endog_vars]
for i in short_var:
    if i in endog.columns: 
        endog_train = endog_train.drop(i, axis = 1)
        
monthly=len(endog.loc["2019-08"])-endog.loc["2019-08"].isna().sum().sum()

# fit the factor model
mod=sm.tsa.DynamicFactorMQ(endog=endog_train, k_endog_monthly=monthly, factors=1, factor_multiplicities=7, factor_orders=2, idiosyncratic_ar1=False)#, standardization=False)
res=mod.fit()

initFactors=res.factors.filtered
initParams=res.params
...
factors=[]
for j in range(181):
    date=df.index.values[576+j]
    endog_train=endog.loc[start:date, endog_vars]
    for i in short_var:
        if i in endog_train.columns: 
            endog_train = endog_train.drop(i, axis = 1)
    mod=sm.tsa.DynamicFactorMQ(endog=endog_train, k_endog_monthly=monthly, factors=1, factor_multiplicities=7, factor_orders=2, idiosyncratic_ar1=False)#, standardization=False)
    ps=res.params
    res=0
    res=mod.fit(start_params=ps)
    forecasts.append(res.forecast()["GDPC1"].values[0])
    factors.append(res.factors.filtered.loc[date].values)
    print(date)
